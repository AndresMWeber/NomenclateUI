...
import nomenclateUI.components.format_widget as format_wgt
from nomenclateUI.components.token_widget import TokenWidgetFactory
from nomenclateUI.components.default import DefaultFrame
...
class InstanceHandlerWidget(DefaultFrame):
    ...
    def generate_name(self, object_item, index, override_dict=None):
        """ Generates a name from the nomenclateUI instance for use in renaming.
        """
    ...
    def update_instance(self, serialized_token_data):
        """ Updates the nomenclateUI instance based on the serialized token widget values/settings
        """
    ...
    def set_format(self):
        """ Sets the current nomenclateUI instance's format.
        """
        if self.input_format.text_utf:
            try:
                ...
                self.refresh()
                self.format_updated.emit(self.NOM.format, self.NOM.format_order, True)
            except nomenclateUI.core.errors.BalanceError as e:
